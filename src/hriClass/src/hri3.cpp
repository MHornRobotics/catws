...
// Creating a class for a turtle2 driver
class turtle2Driver {
 private:
 ...
 public:
  ...
  // publisher
  turtle2Driver(ros::NodeHandle& nh) {
    n = nh;
    velPub = n.advertise<geometry_msgs::Twist>("/turtle1/cmd_vel", 1);
  }

  void lead() {
    std::cout << "Will now try to lead the way for my turtle1!\n";
...
// Creating a that will attempt to follow the turtle2
class turtle1Driver {
 private:
  // Nothing outside of this class should even need to see this
  ros::NodeHandle n;
  ros::Publisher velPub;
  ros::Subscriber speechDecision;
  turtlesim::Pose currentPose;
  geometry_msgs::Twist command;
  float theta;
  std_msgs::String currentCommand;

 public:
  // Creating a constructor that takes a ROS node handle and generates a
  // publisher
  void speechCallback(const turtlesim::Pose& msg) {
    // SET CURRENTCOMMAND.DATA towhat the current job is
  }

  turtle1Driver(ros::NodeHandle& nh) {
    n = nh;
    currentCommand.data = "Null";
    velPub = n.advertise<geometry_msgs::Twist>("/turtle2/cmd_vel", 1);
    speechDecision = n.subscribe("/hlpr_speech_commands", 10,
                                 &turtle1Driver::speechCallback, this);
  }

  bool follow() {
    std::cout << "Will now try to walk past turtle2.\n";
    ros::Duration(0.5).sleep();
    ...
    while (n.ok()) {
      // Implementing Forcefield around the turtle2, eggshell size
...
int main(int argc, char** argv) {
  // init the ROS node
  ros::init(argc, argv, "turtle2turtle1");
  ros::NodeHandle nh;

  // Creates and runs the drunken turtle
  turtle1Driver turtle1(nh);
  turtle2Driver turtle2(nh);
  ...
  boost::thread thread_b(&turtle2Driver::lead, turtle2);
}
